...
def log_debug(msg: str):
    ...
    try:
        with open("debug_mcp.log", "a", encoding="utf-8") as f:
            f.write(f"[{datetime.now().isoformat()}] {msg}\n")
    except Exception:
        pass


def _sync_crawl(url: str):
    import asyncio
    import sys

    log_debug("_sync_crawl started for: " + url)
    # Run the async crawler in a new event loop inside this thread
    async def _crawl():
        log_debug("AsyncWebCrawler instantiating...")
        async with AsyncWebCrawler(verbose=False) as crawler:
            log_debug("AsyncWebCrawler instantiated, running arun...")
            res = await crawler.arun(url=url)
            log_debug("AsyncWebCrawler arun complete!")
            return res

    original_stdout = sys.stdout
    sys.stdout = sys.stderr
    try:
        log_debug("Creating new event loop inside thread...")
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        log_debug("Running event loop until complete...")
        r = loop.run_until_complete(_crawl())
        log_debug("Event loop run complete, closing loop...")
        loop.close()
        log_debug("Loop closed, returning crawl result.")
        return r
    except Exception as e:
        log_debug("Exception in _sync_crawl thread: " + str(e))
        raise e
    finally:
...
async def _crawl4ai_fetch(url: str) -> dict:
    ...
    log_debug("_crawl4ai_fetch called for: " + url)
    loop = asyncio.get_running_loop()
    log_debug("Dispatching to ThreadPoolExecutor...")
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        r = await loop.run_in_executor(executor, _sync_crawl, url)
    log_debug("_crawl4ai_fetch thread execution complete!")
    ...
    text = str(raw)
    log_debug("Markdown processed, returning dict of size " + str(len(text)))
    return {
